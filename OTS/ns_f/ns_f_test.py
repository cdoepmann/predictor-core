...
setup_dict = {}
setup_dict['v_max'] = 200000  # packets / s
setup_dict['s_max'] = 2000  # packets
...
nw.linear_growth_threshold = 1000
...
for k in range(n_steps):
    if k < 400 and input_1.s_max-input_1.s >= 300:
        input_1.add_2_buffer(buffer_ind=0, circuit=0, n_packets=300, tnow=nw.t)
    if k < 400 and input_2.s_max-input_2.s >= 300:
        input_2.add_2_buffer(buffer_ind=0, circuit=1, n_packets=300, tnow=nw.t)
...
dat.packet_list[dat.packet_list['ttransit'] != np.inf].hist(by='circuit', column='ttransit', figsize=[13, 5], ax=ax2)
plt.show()
